...
export function UserProfile({ user }: UserProfileProps) {
  ...
  // Fetch real history from database when history tab is selected
  useEffect(() => {
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-green-600 rounded-2xl p-8 text-white mb-6 shadow-lg">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white/10 backdrop-blur rounded-xl p-4">
            ...
            <div className="text-3xl font-bold">{user.totalRecycled || 0}</div>
          </div>
          <div className="bg-white/10 backdrop-blur rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <div className="text-3xl font-bold">{(user.co2Saved || 0).toFixed(1)} kg</div>
          </div>
        </div>
      </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
                    ...
                    <div className="text-2xl font-bold text-green-600">
                      {((user.totalRecycled || 0) * 0.5).toFixed(1)} kg
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-4 border border-blue-200">
                    ...
                    <div className="text-2xl font-bold text-blue-600">
                      {((user.totalRecycled || 0) * 2).toFixed(0)} L
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-4 border border-purple-200">
                    ...
                    <div className="text-2xl font-bold text-purple-600">
                      {((user.totalRecycled || 0) * 5).toFixed(0)} kWh
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-xl p-4 border border-orange-200">
                    ...
                    <div className="text-2xl font-bold text-orange-600">
                      {((user.totalRecycled || 0) * 0.15).toFixed(1)}
                    </div>
                  ...
                  <div className="relative">
                    <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${Math.min(((user.points || 0) / 400) * 100, 100)}%` }}
                      ></div>
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center text-xs font-bold text-white">
                      {user.points || 0} / 400 points
                    </div>
                  </div>
                  <div className="text-sm text-gray-600 mt-2">
                    {400 - (user.points || 0) > 0
                      ? `${400 - (user.points || 0)} more points to unlock!`
                      : 'Reward unlocked! 🎉'}
                  </div>
                </div>
              </div>
                ...
                {REWARDS.map(reward => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {canAfford ? 'Redeem' : `Need ${reward.points - (user.points || 0)} more`}
                      </button>
                    </div>
                  );
                })}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-sm text-gray-600">${entry.value || 0} value</div>
                          <div className="text-xs text-green-600">{(entry.co2Saved || 0).toFixed(1)} kg CO₂ saved</div>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            { rank: 3, name: 'Emma Davis', points: 890, icon: Award },
            { rank: 4, name: 'Alex Kim', points: 780, icon: Zap },
            { rank: 5, name: user.name + ' (You)', points: user.points, icon: Star },
          ].map(entry => {
